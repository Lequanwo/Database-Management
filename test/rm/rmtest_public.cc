...
//            int tableId;
//            PeterDB::RID rid;
//            rm.getTableIdFromTableTable(tableName, tableId, rid);
//            printf("%d \n", tableId);
//        }

        //ASSERT_EQ(rm.deleteCatalog(), success)<< "deleteCatalog() should success ";
    ...
    TEST_F(RM_Tuple_Test, get_attributes) {
        ...
        if(fileExists(TABLES_TABLE))
            std::cout << "TABLES_TABLE" << std::endl;
    ...
    TEST_F(RM_Scan_Test, simple_scan) {
        ...
        ASSERT_EQ(0, success) << "while loop";
    ...
    TEST_F(RM_Scan_Test, simple_scan_after_table_deletion) {
        ...
        // Set up the iterator
        std::vector<std::string> attributes{"age"}; // Age or age ???
...
*/
    ...
    TEST_F(RM_Large_Table_Test, insert_large_tuples) {
        ...
        int numTuples = 5000; // 5000 as default

        // GetAttributes
        ASSERT_EQ(rm.getAttributes(tableName, attrs), success) << "RelationManager::getAttributes() should succeed.";

        // Initialize a NULL field indicator
        nullsIndicator = initializeNullFieldsIndicator(attrs); // attr0 - attr29

        // Insert numTuples tuples into table
        for (int i = 0; i < numTuples; i++) {
            if(i % 500 == 0){
                std::cout << "[Insert numTuples tuples into table] number of i is: " <<std::to_string(i) << std::endl;
            }
            if(i == 1018){
                std::cout << "[Insert numTuples tuples into table] number of i is: " <<std::to_string(i) << std::endl;
            }
            ...
            ASSERT_EQ(rm.insertTuple(tableName, inBuffer, rid), success)
                                        << "RelationManager::insertTuple() should succeed.";
//            std::cout << "Pagenum: "<< std::to_string(rid.pageNum) << " slotNum: "  << std::to_string(rid.slotNum) << std::endl;
    ...
    TEST_F(RM_Large_Table_Test, update_and_read_large_tuples) {
        ...
        int numTuples = 5000; // 5000
        unsigned numTuplesToUpdate1 = 2000; // 2000
        unsigned numTuplesToUpdate2 = 2000; // 2000
        inBuffer = malloc(bufSize);
        outBuffer = malloc(bufSize);
...
//            std::cout << "####################################################################################" << std::endl;
//            std::cout << "### first numTuplesToUpdate1  number of i is:  " <<std::to_string(i) << std::endl;
            if(i % 1000 == 0){
                std::cout << "[Update the first numTuplesToUpdate1 tuples] value of i is: " <<std::to_string(i) << std::endl;
            }
            memset(inBuffer, 0, bufSize);
            rid = rids[i];
            ...
            ASSERT_EQ(rm.updateTuple(tableName, inBuffer, rid), success) << "first RelationManager::updateTuple() should succeed.";
        ...
        std::cout << "************************************************************************" << std::endl;
        std::cout << "****** Update the first numTuplesToUpdate1 tuples FINISHED *****"  << std::endl;
        std::cout << "************************************************************************" << std::endl;

        // Update the last numTuplesToUpdate2 tuples
        for (unsigned i = numTuples - numTuplesToUpdate2; i < numTuples; i++) {
//            std::cout << "####################################################################################" << std::endl;
//            std::cout << "####### the last numTuplesToUpdate2 number of i is:  " << std::to_string(i) << std::endl;
            if(i % 1000 == 0){
                std::cout << "[Update the last numTuplesToUpdate2 tuples] Value of i is: " <<std::to_string(i) << std::endl;
            }
            memset(inBuffer, 0, bufSize);
            ...
            ASSERT_EQ(rm.updateTuple(tableName, inBuffer, rid), success) << "second RelationManager::updateTuple() should succeed.";
...
//            std::cout << "####################################################################################" << std::endl;
//            std::cout << "###### number of i is:  " << std::to_string(i) << std::endl;
            memset(inBuffer, 0, bufSize);
            memset(outBuffer, 0, bufSize);
            prepareLargeTuple(attrs.size(), nullsIndicator, i + 10, inBuffer, size);
        ...
        std::cout << "************************************************************************" << std::endl;
        std::cout << "****** Read the first numTuplesToUpdate1 tuples FINISHED *****"  << std::endl;
        std::cout << "************************************************************************" << std::endl;
        ...
        std::cout << "************************************************************************" << std::endl;
        std::cout << "****** Read the last numTuplesToUpdate2 tuples FINISHED *****"  << std::endl;
        std::cout << "************************************************************************" << std::endl;
...
//            std::cout << "####################################################################################" << std::endl;
//            std::cout << "### [Delete the first numTuplesToDelete tuples] number of i is:  " <<std::to_string(i) << std::endl;
//            std::cout << "### Pagenum: "<< std::to_string(rids[i].pageNum) << " slotNum: "  << std::to_string(rids[i].slotNum) << std::endl;
        ...
        std::cout << "" << std::endl;
        std::cout << "************************************************************************" << std::endl;
        std::cout << "****** [Delete the first numTuplesToDelete tuples] FINISHED *****"  << std::endl;
        std::cout << "************************************************************************" << std::endl;
        std::cout << "" << std::endl;
        ...
        for (unsigned i = 0; i < numTuplesToDelete; i++) {
//            std::cout << "####################################################################################" << std::endl;
            // std::cout << "### [Try to read the first numTuplesToDelete deleted tuples]  should not succeed" << std::endl;
        ...
        std::cout << "" << std::endl;
        std::cout << "************************************************************************" << std::endl;
        std::cout << "****** [Try to read the first numTuplesToDelete deleted tuples] FINISHED *****"  << std::endl;
        std::cout << "************************************************************************" << std::endl;
        std::cout << "" << std::endl;
        std::cout << "" << std::endl;
...
//            std::cout << "####################################################################################" << std::endl;
//            std::cout << "### [Read the non-deleted tuples] first numTuplesToUpdate1  number of i is:  " <<std::to_string(i) << std::endl;
            ASSERT_EQ(rm.readTuple(tableName, rids[i], outBuffer), success)
                                        << "RelationManager::readTuple() should succeed.";
    ...
    TEST_F(RM_Large_Table_Test, scan_large_tuples) {
        ...
        while (rmsi.getNextTuple(rid, outBuffer) != RM_EOF) {
            ...
            offset += 4; // length of varchar

            auto *attr15 = (uint8_t *) malloc(size + 1);
            memcpy(attr15, (uint8_t *) outBuffer + offset + nullAttributesIndicatorActualSize, size); // real char
            attr15[size] = 0;
            offset += size;
            char target;
